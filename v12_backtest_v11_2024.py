#!/usr/bin/env python3
"""
V11_IC_Optimized 2024年全年回测 (Q2-Q4)
=========================================
"""
...
print("="*70)
print("V11_IC_Optimized 2024年全年回测 (Q2-Q4)")
print("="*70)
...
# 加载2024年Q2-Q4数据（提前2个月用于计算因子）
sql = """
SELECT code, trade_date, close, turnover, amount
FROM stock_kline 
WHERE trade_date BETWEEN '2024-03-01' AND '2024-12-31'
AND amount >= 500000
...
# 获取2024年Q2-Q4的交易日（从4月开始，因为Q1已有数据）
all_dates = df[df['trade_date'] >= '2024-04-01']['trade_date'].unique()
selection_dates = sorted(all_dates)[::3]  # 每3天
...
if results:
    ...
    run_id = 'V11_IC_2024_Q2Q4_' + datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    ''', (
        run_id, 'V11_IC_Optimized', '2024-04-01', '2024-12-31', 1000000,
    ...
    df_results.to_csv(f'backtest_results/v11_ic/trades_2024_q2q4_{run_id}.csv', index=False)
