...
print("="*70)
print("V11_IC_Optimized 2025年Q2回测")
print("="*70)
...
# 加载2025年Q2数据（提前2个月用于计算因子）
sql = """
SELECT code, trade_date, close, turnover, amount
FROM stock_kline 
WHERE trade_date BETWEEN '2025-02-01' AND '2025-06-30'
AND amount >= 500000
...
# 获取2025年Q2的交易日
all_dates = df[(df['trade_date'] >= '2025-04-01') & (df['trade_date'] <= '2025-06-30')]['trade_date'].unique()
...
if results:
    ...
    run_id = 'V11_IC_2025Q2_' + datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    ''', (
        run_id, 'V11_IC_Optimized', '2025-04-01', '2025-06-30', 1000000,
    ...
    df_results.to_csv(f'backtest_results/v11_ic/trades_2025q2_{run_id}.csv', index=False)
