...
class StockScreenerV9:
    ...
    def _load_all_stocks(self) -> List[str]:
        """加载全A股列表（5486只）"""
        # 尝试多个路径
        # 项目根目录（版本B在versions/version_b/下，需要向上两级）
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        possible_paths = [
            os.getenv('STOCK_LIST_FILE'),  # 环境变量
            os.path.join(project_root, 'data', 'all_a_stocks.txt'),  # 项目本地
    ...
    def screen(self, top_n: int = 3) -> tuple:
        ...
        for code, data in filtered_quotes.items():
            ...
            # DeepSeek优化版硬性过滤条件
            if not self._pass_deepseek_filters(data, historical_prices):
                continue
    ...
    def _pass_deepseek_filters(self, data: Dict, historical_prices: List[float] = None) -> bool:
        """
        DeepSeek优化版硬性过滤条件
        ...
        # 1. 当日涨幅过滤 (盘前选股时，change_pct是相对于昨日收盘的涨幅)
        # 注意：盘前选股时，change_pct是集合竞价涨幅，需要限制
        if change_pct > 7:
            logger.debug(f"{code} 当日涨幅{change_pct:.2f}% > 7%，过滤")
            return False
        
        # 2. 60日价格分位过滤
        if historical_prices and len(historical_prices) >= 60:
            current_price = historical_prices[-1]
            min_price = min(historical_prices[-60:])
            max_price = max(historical_prices[-60:])
            if max_price > min_price:
                price_percentile = (current_price - min_price) / (max_price - min_price) * 100
                if price_percentile > 85:
                    logger.debug(f"{code} 60日价格分位{price_percentile:.1f}% > 85%，过滤")
                    return False
        
        # 3. 成交额过滤 (过去20日日均成交额 ≥ 1.5亿)
        # 注意：这里简化处理，使用当日成交额作为 proxy
        # 实际应该使用历史平均成交额
        if turnover > 0 and turnover < 1.5e8:
            logger.debug(f"{code} 成交额{turnover/1e8:.2f}亿 < 1.5亿，过滤")
            return False
...
class MarketTiming:
    ...
    def analyze(self, index_quotes: Dict = None, quotes: Dict = None) -> Dict:
        ...
        try:
            ...
            # 动态阈值调整（连续变化）- DeepSeek优化版
            # 新公式：threshold = 60 + (market_strength - 60) × 0.3
            # 新逻辑：市场越强（>60），阈值越高（>60），选股更严格，避免追高
            base_threshold = 60
            threshold = base_threshold + (self.market_strength - 60) * 0.3
...
class MultiFactorAnalyzerV10:
    ...
    def calculate_score(self, data: Dict, sectors: List[Dict], 
                       ...
                       sentiment_factor: Dict = None) -> Dict:
        """
        计算多因子评分 - V10+P1+P2+舆情完整版 (DeepSeek优化版)
        
        因子权重（DeepSeek优化后）：
        - 技术: 15% (0-15分) - 降低，避免过度追涨
        - 情绪: 3% (0-3分) - 技术情绪，简化
        - 板块: 22% (0-22分) - 降低，避免过度追逐热点
        - 资金: 18% (0-18分) - 微调
        - 风险: 22% (0-22分) - 大幅提升，强化风控
        - 一致预期: 12% (0-12分) - 大幅提升，增加基本面锚定
        - 舆情: 8% (0-8分) - 提升，增加另类数据权重
        """
        price = data.get('price', 0)
        change_pct = data.get('change_pct', 0)
        volume = data.get('volume', 0)
        ...
        # 优化后的权重计算（DeepSeek优化版）
        # 新权重：技术15 + 情绪3 + 板块22 + 资金18 + 风险22 + 一致预期12 + 舆情8 = 100分

        # 1. 基础权重调整
        tech_adjusted = tech_score * 0.75        # 20分制 -> 15分制 (×0.75)
        sentiment_adjusted = sentiment_score * 0.3  # 10分制 -> 3分制 (×0.3)
        sector_adjusted = sector_score * 0.6286  # 35分制 -> 22分制 (×0.6286)
        money_flow_adjusted = money_flow_score * 0.9  # 20分制 -> 18分制 (×0.9)
        risk_adjusted = risk_score * 1.467  # 15分制 -> 22分制 (×1.467)
        consensus_adjusted = consensus_score * 2.4  # 5分制 -> 12分制 (×2.4)
        news_sentiment_adjusted = news_sentiment_score * 1.6  # 5分制 -> 8分制 (×1.6)
...
class ReportGeneratorV10Plus:
    ...
    def generate_premarket_report(self, picks: List[Dict], quotes: Dict = None, strategy_results: Dict = None) -> str:
        ...
        lines.extend([
            ...
            "======================================================================",
            f"基于全A股多因子评分（P0优化版）",
            f"因子权重: 技术15% | 情绪3% | 板块22% | 资金18% | 风险22% | 一致预期12% | 舆情8% (DeepSeek优化版)",
            "----------------------------------------------------------------------",
            "",
        ])
...
class StockAnalysisSystem:
    ...
    def run_intraday(self, mode: str = 'noon', send: bool = True) -> Dict:
        ...
        print("📈 因子权重: 技术15% | 情绪3% | 板块22% | 资金18% | 风险22% | 一致预期12% | 舆情8% (DeepSeek优化版)")
        print("="*80)
