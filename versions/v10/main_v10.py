...
class StockScreenerV9:
    ...
    def screen(self, top_n: int = 3) -> tuple:
        ...
        # P0新增：Step 0 - 大盘择时分析
        logger.info("Step 0: 大盘择时分析")
        market_context = self.market_timing.analyze()
        ...
        # 根据市场环境调整选股数量
        adjusted_top_n = min(top_n, market_context.get('max_picks', 3))
        ...
        for code, data in filtered_quotes.items():
            ...
            # 应用动态阈值
            threshold = score.get('threshold', 65)
        ...
        top_stocks = results[:adjusted_top_n]

        logger.info(f"V10选股完成，选出 {len(top_stocks)} 只")
        for s in top_stocks:
    ...
    def _get_historical_prices(self, code: str) -> List[float]:
        """
        P1+P2：获取历史价格数据（使用Baostock/AkShare）
        ...
        # 使用历史数据提供者获取
        try:
            prices = self.hist_data_provider.get_stock_history(code, days=60)
            if prices:
                self.historical_data_cache[code] = prices
                logger.debug(f"✅ 获取 {code} 历史数据: {len(prices)} 天")
                return prices
        except Exception as e:
...
class MarketTiming:
    ...
    def analyze(self, index_quotes: Dict = None) -> Dict:
        """
        分析大盘环境
        
        Returns:
            {
                'is_bear_market': bool,  # 是否熊市/弱势
                'threshold_adjust': int,  # 阈值调整
                'max_picks': int,  # 最大选股数量
                'reason': str  # 原因说明
            }
        """
        try:
            # 获取沪深300数据
            hs300 = self._get_hs300_data(index_quotes)
            
            if not hs300:
                return {'is_bear_market': False, 'threshold_adjust': 0, 'max_picks': 3, 'reason': '无法获取大盘数据'}
            
            # 计算关键指标
            price = hs300.get('price', 0)
            change_pct = hs300.get('change_pct', 0)
            
            # 获取历史数据计算MA20（简化版，实际应该从缓存获取）
            # 这里用当日涨跌幅作为简单判断
            ...
            if change_pct < -2:
                bear_signals += 1
                reasons.append(f"大盘跌幅{change_pct:.2f}%")
            
            # 信号2：大盘跌幅超过-1%且上涨家数<30%（这里简化判断）
            if change_pct < -1:
                bear_signals += 1
                reasons.append("大盘明显下跌")
            
            # 判断结果
            if bear_signals >= 2:
                self.is_bear_market = True
                self.reason = "弱势市场: " + "; ".join(reasons)
                return {
                    'is_bear_market': True,
                    'threshold_adjust': 10,  # 阈值从65提高到75
                    'max_picks': 1,  # 最多选1只
                    'reason': self.reason
                }
            elif bear_signals >= 1:
                self.is_bear_market = False
                self.reason = "偏谨慎: " + "; ".join(reasons)
                return {
                    'is_bear_market': False,
                    'threshold_adjust': 5,  # 阈值从65提高到70
                    'max_picks': 2,  # 最多选2只
                    'reason': self.reason
                }
            else:
            ...
            return {'is_bear_market': False, 'threshold_adjust': 0, 'max_picks': 3, 'reason': f'分析异常: {e}'}
...
class MultiFactorAnalyzerV10:
    ...
    核心优化：
    1. 技术因子从"当日涨"转向"趋势确认"（增加MA、突破、RSI）- P0
    2. 情绪因子简化合并（去除与技术的重复）- P0
    3. 增加大盘择时模块 - P0
    4. 成交量结构精细化 - P0
    5. 板块轮动深化（RPS、双周期动量）- P1
    6. 风险因子扩展（回撤、历史分位）- P1
    7. 资金流向精细化（主力、大单、北向）- P2
    8. 一致性预期因子（机构调研、评级）- P2
    ...
    def calculate_score(self, data: Dict, sectors: List[Dict], 
                       ...
                       sector_rps_data: Dict = None) -> Dict:
        """
        计算多因子评分 - V10+P1+P2完整版
        ...
        # 计算总分（含P2一致预期）
        total = (tech_score + sentiment_score + sector_score + 
                money_flow_score + risk_score + consensus_score)
        ...
        if market_context:
            threshold += market_context.get('threshold_adjust', 0)
            max_picks = market_context.get('max_picks', 3)
    ...
    def _calc_sentiment_score_v10(self, change_pct: float, volume: float,
        ...
        """情绪因子评分 V10 (0-10分) - 简化版"""
        score = 5  # 基础分
        
        # 计算换手率（简化估算）
        # 假设流通市值，主板平均约50亿，中小创约20亿
        if code.startswith(('00', '60')):
            estimated_cap = 50e8  # 50亿
        else:
            estimated_cap = 20e8  # 20亿
...
class StockAnalysisSystem:
    ...
    def run_premarket(self, top_n: int = 3, send: bool = True) -> Dict:
        ...
        """
        print("\n" + "="*80)
        print(f"🚀 股票分析系统 1.2 (V10-P0优化) - 盘前分析")
        print("="*80)

        # 1. V10选股
        logger.info("Step 1: V10选股")
        ...
        if self.strategy_manager:
            logger.info("Step 2: 运行三策略选股")
            try:
                # 准备板块数据
                from sector_first_screener import SectorFirstScreener
                screener = SectorFirstScreener()
                sectors_data = {}
                for sector_name, codes in screener.SECTOR_MAP.items():
                    sectors_data[sector_name] = [{'code': c} for c in codes[:5]]
                ...
                for pick in picks:
                    ...
                    if sector != '其他':
                        top_sectors.append({
                            'name': sector,
                            'codes': screener.SECTOR_MAP.get(sector, [])
                        })
                ...
                logger.info(f"✅ 策略选股完成: 突破{len(strategy_results.get('breakout', []))}只 动量{len(strategy_results.get('momentum', []))}只")
            except Exception as e:
                logger.warning(f"⚠️ 策略选股失败: {e}")

        # 3. 生成V10+报告（包含策略信号）
        logger.info("Step 3: V10+生成报告")
        report = self.reporter.generate_premarket_report(picks, quotes, strategy_results)

        # 4. 保存报告
        logger.info("Step 4: 保存报告")
        filepath = self.storage.save(report, 'premarket')

        # 5. 发送推送
        if send:
            logger.info("Step 5: 发送推送")
            self.notifier.send_in_parts("🌅 盘前选股报告 1.0", report)
        
        print(f"\n✅ 盘前分析完成: {len(picks)}只股票")
        if strategy_results:
            print(f"   突破策略: {len(strategy_results.get('breakout', []))}只")
            print(f"   双动量: {len(strategy_results.get('momentum', []))}只")
        print("="*80)
    ...
    def run_postmarket(self, picks: List[Dict] = None, send: bool = True) -> Dict:
        ...
        # 获取今日开盘价数据（用于计算推荐股涨跌幅）
        logger.info("Step 1a: 获取今日开盘价数据")
        opening_prices = {}
        if picks:
            try:
                from sina_finance_api import SinaFinanceAPI
                ...
                if not df.empty:
                    ...
                    logger.info(f"✅ 获取到 {len(opening_prices)} 只股票的今日开盘数据")
            except Exception as e:
                logger.warning(f"⚠️ 获取开盘价失败: {e}")
        ...
        if all_stocks:
            ...
            if picks:
                ...
                logger.info(f"✅ 获取到 {len(opening_prices)} 只推荐股的开盘数据")
    ...
    def run_intraday(self, mode: str = 'noon', send: bool = True) -> Dict:
        ...
        print(f"\n" + "="*80)
        print(f"🕐 {title} 1.2 (V10-P0优化) - 上午收盘总结 + 下午选股")
        print("="*80)
        ...
        if all_stocks:
            ...
            logger.info(f"✅ V10盘中选股完成，选出 {len(afternoon_picks)} 只")
    ...
    def _select_afternoon_opportunities_v10(self, stocks: List, strong_sectors: List[Dict], 
        ...
        adjusted_top_n = market_context.get('max_picks', 3) if market_context else 3
        threshold = market_context.get('threshold_adjust', 65) if market_context else 65
        ...
        for stock in stocks:
            ...
            data = {
                ...
                'volume': stock.volume,
                'turnover': getattr(stock, 'turnover', stock.volume * stock.price),
            ...
            # P0优化：使用V10多因子评分
            historical_prices = []  # 盘中简化处理，不获取历史数据
            ...
            # 应用动态阈值
            final_threshold = score.get('threshold', threshold)
        ...
        if afternoon_picks:
            for i, pick in enumerate(afternoon_picks, 1):
                ...
                lines.extend([
                    ...
                    f"   - 当前价: ¥{pick.get('price', 0):.2f} ({change_pct:+.2f}%)",
                    f"   - 综合评分: **{score:.1f}**",
                    f"   - 因子得分: 技术{factors.get('technical', 'N/A')} | 情绪{factors.get('sentiment', 'N/A')} | 板块{factors.get('sector', 'N/A')} | 资金{factors.get('money_flow', 'N/A')} | 风险{factors.get('risk', 'N/A')}",
