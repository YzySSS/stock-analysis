...
function renderStrategyCards(data) {
  ...
  qs('#strategies-current-count').textContent = String(data.summary?.current_count ?? 0);
  qs('#strategies-legacy-count').textContent = String(data.summary?.legacy_count ?? 0);
  ...
  list.innerHTML = items.map((item) => `
    <article class="strategy-card ${item.id === currentStrategyId ? 'selected' : ''}" data-strategy-card="${escapeHtml(item.id)}">
      ...
      <div class="muted">标签：${(item.tags || []).map((tag) => escapeHtml(tag)).join(' / ') || '暂无'} · ${item.executable === false ? '仅展示，不可执行' : '可纳入当前执行链路'}</div>
...
function renderStrategyDetail(strategy = null) {
  ...
  const note = strategy.executable === false
...
function renderStrategyFactors(strategy = null) {
  ...
  summary.textContent = strategy.executable === false
    ? `当前策略为 ${strategy.display_name || strategy.id}，因尚未接入执行链路，当前展示以配置说明为主。`
