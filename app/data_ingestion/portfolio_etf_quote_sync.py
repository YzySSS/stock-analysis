...
def save_snapshot(code: str, rows: list[dict[str, Any]], batch_id: str | None = None) -> int:
    if not rows:
        return 0
    latest = rows[-1]
    snapshot_batch_id = batch_id or f"portfolio_etf_quote_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    with mysql_conn() as conn:
        with conn.cursor() as cursor:
