...
## 日线、量能与筹码前移到选股评分（2026-07-20）

本轮将原本主要用于入选后交易计划的技术信息，前移到 `a_share_sentiment` 综合评分阶段。产品边界明确为：**只参与评分，不新增硬过滤**。强舆情股票即使技术结构偏弱，仍保留候选资格，但综合得分会反映资金交易结构的差异。

新增/调整因子：

- `daily_trend`（8%）：结合实时/最新价格与 MA5、MA20、MA30 的位置，MA5/MA10 与 MA20/MA30 的排列，以及相对 MA5 的偏离程度评分。
- `volume_confirm`（6%）：从单一量比升级为量比、相对 5 日成交均值、相对 20 日成交均值的组合确认；放量滞涨继续降分。
- `chip_structure`（6%）：结合现价相对筹码中枢、15%-85% 成本带宽、获利盘比例和上沿压力评分。

解释口径：

- 日线状态展示为 `日线趋势确认 / 日线突破修复 / 日线偏离过大 / 日线趋势偏弱 / 日线结构中性`。
- 筹码状态展示为 `筹码结构有支撑 / 筹码中枢下方 / 获利盘压力偏高 / 筹码结构中性 / 筹码数据待补`。
- 技术结构偏弱只写入风险说明并降低综合得分，不在 `prepare_context()` 或 `select()` 增加淘汰条件。
- 缺少均线或筹码数据时对应因子按 50 分中性处理，避免数据缺口被误判为负面基本事实。

策略版本由 `0.1.0` 升级为 `0.2.0`，用于区分历史结果口径。

## 可交易级、风险收益与大盘指数环境（2026-07-20）

本轮在不改变“舆情主导、技术和市场环境软评分”的前提下，增加交易层风险约束。策略版本升级为 `0.3.0`。

### 1. 同主题最多一只可交易级

- 综合评分和最终排名不变，不因主题重复删除候选。
- 候选股需同时满足盘中交易确认、日线确认/突破、筹码结构可接受和交易计划合规，才可进入“可交易级”。
- 同一 `opinion_sector_name` 中，只保留最终排名最高的一只合格标的为“可交易级”。
- 同主题后续股票降为“观察级”，并显式说明名额已被排名更高的标的占用；仍保留在结果池中，不构成选股硬过滤。

### 2. 统一短线风险收益

- `a_share_sentiment` 新交易计划统一使用入选价下方约 `5%` 的计划止损，技术支撑位保留为盘中观察依据。
- 第一止盈必须使 `(止盈价 - 买入价) / (买入价 - 止损价) >= 1.2`。
- 校验不只以入选价计算，还以建议买入区间上沿做最差情形校验；必要时收紧买入上沿，避免页面显示 `1.2` 但在区间高位买入后实际不达标。
- 交易计划版本为 `selection_trade_plan_v3_risk_control`，保留已保存历史计划的原口径。

### 3. 市场环境显式纳入宽基指数

`market_context` 因子总权重从 `2%` 提高到 `5%`，仍然只做软评分。因子内部口径为：

- 沪深300、中证500、中证1000平均趋势分：`40%`。
- 三只宽基指数当日平均表现：`20%`。
- 全 A 股上涨宽度：`25%`。
- 三只宽基指数平均量能：`15%`。

数据缺失时按现有分量重新归一化；若宽基分量全部缺失，才回退旧 `market_strength`。日更脚本默认串行同步三只宽基指数，保留 `--index-code` 用于指定单一/多个指数。

2026-07-20 以最新交易日 `2026-07-17` 实跑验证：沪深300、中证500、中证1000分别约 `-3.60% / -5.55% / -6.08%`，聚合市场环境分约 `15.79`，能正确识别当日宽基普跌压力。真实全市场本地选股验证中，3 条结果全部生成约 `5%` 止损且第一止盈盈亏比 `>=1.2`；同一“消费”主题的第二只标的被降为观察级，结果数量保持不变。

以上完成的是风险约束与解释闭环，不等于策略已通过收益验证；策略仍保持 `experimental / unvalidated`。
