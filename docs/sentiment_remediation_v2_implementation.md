...
> 历史状态说明（2026-08-10）：本文记录 `0.5.0` 初始实现。该版本的历史
> 证据因资金分子/分母时钟混用标记为 `contaminated_retained`，未被改写；
> 当前 `0.5.1` 只修复同钟计算，并从 2026-08-11 重新开始 30 个交易日的
> 配对观察，仍保持 `shadow_only / unvalidated`。
