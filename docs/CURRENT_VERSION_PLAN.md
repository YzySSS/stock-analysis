...
| 自动回归 | 281 项通过 |
| 运行状态 | 2026-07-21 16:24 readiness `degraded`：日线已到 07-21、因子输入暂到 07-20；3 个 worker healthy/idle，3 类队列均为 0 |
...
- `a_share_sentiment 0.4.4`：已修复股票—板块关系、局部新闻方向与局部事件时效、主题资金映射、候选截断、深 V 分级、公告日基本面、股票简称中文子串伪命中、相邻子句串线、历史缓存绕过和旧催化冒充今日买点；旧版结果保留但退出有效统计。首个正式观察 `asent_fwd_20260721_v044` 已于 2026-07-21 16:21 成功完成，3 只均为观察级，策略仍保持 `unvalidated`。
