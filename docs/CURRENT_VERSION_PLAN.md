...
当前源代码基线已推进至提交 `a7c83ed`；其后的版本计划文档提交不改变线上业务语义。
...
| 策略状态 | 11 条可加载、7 条数据就绪、2 条普通选股可执行、2 条仅研究回测、0 条通过交易有效性验证；A 股舆情为 `0.4.0` |
...
- `a_share_sentiment 0.4.0`：已修复股票—板块关系、新闻方向、主题资金映射、候选截断、深 V 分级和公告日基本面口径；旧版结果保留但退出有效统计，0.4.0 从新协议重新积累前瞻样本，仍保持 `unvalidated`。
...
| A 股舆情前瞻观察 | 已完成语义整改 | 0.4.0 重新冻结关系、方向、资金、深 V 与 PIT 基本面口径；旧结果可追溯但不混入新统计 |
...
1. **FWD-OBS 前瞻样本积累**：从 `a_share_sentiment 0.4.0` 部署日起按新协议重新采集；窗口内只采集和展示，不调参数、不提前宣布验证通过。
...
- [`a_share_sentiment_forward_observation_v0_4_0_2026-07-21.md`](./a_share_sentiment_forward_observation_v0_4_0_2026-07-21.md)：A 股舆情 0.4.0 的语义修正、证据重置与固定前瞻观察合同。
