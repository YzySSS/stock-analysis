...
| 策略状态 | 11 条可加载、7 条数据就绪、2 条普通选股可执行、2 条仅研究回测、0 条通过交易有效性验证；A 股舆情为 `0.3.1` |
...
- `a_share_sentiment 0.3.1`：从 2026-07-21 起按固定 60 分底线、最多 3 只和收盘后 16:20 时点前瞻观察；20 天或 50 个候选只进入 `preliminary_ready`，仍保持 `unvalidated`。
...
| A 股舆情前瞻观察 | 已完成工程闭环 | 固定协议、零候选日、AI/fallback 分段、1/3/5/20 日收益、三宽基超额、MFE/MAE 与不可变源码标签 |
