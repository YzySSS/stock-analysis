...
print("="*70)
print("V11_IC_Optimized 2025年全年回测")
print("="*70)
...
# 加载2025年数据（提前2个月用于计算因子）
sql = """
SELECT code, trade_date, close, turnover, amount
FROM stock_kline 
WHERE trade_date BETWEEN '2024-11-01' AND '2025-12-31'
AND amount >= 500000
...
# 获取2025年的交易日
all_dates = df[df['trade_date'] >= '2025-01-01']['trade_date'].unique()
...
if results:
    ...
    run_id = 'V11_IC_2025_' + datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    ''', (
        run_id, 'V11_IC_Optimized', '2025-01-02', '2025-12-31', 1000000,
    ...
    df_results.to_csv(f'backtest_results/v11_ic/trades_2025_{run_id}.csv', index=False)
