...
35 18 * * 1-5 cd $PROJECT_ROOT && PYTHONPATH=$PROJECT_ROOT $PYTHON_BIN scripts/run_stock_status_pit_backfill.py --stages suspensions --suspension-recent-trade-days 3 --skip-feature-cache --pause-seconds 0 >> $LOG_DIR/stock_status_pit_backfill.log 2>&1"
...
FORWARD_OBSERVATION_JOB="20 16 * * 1-5 cd $PROJECT_ROOT && PYTHONPATH=$PROJECT_ROOT $PYTHON_BIN scripts/run_strategy_forward_observation.py >> $LOG_DIR/strategy_forward_observation.log 2>&1"
