...
DATABASE_BACKUP_JOB="30 0 * * * flock -xn /tmp/stock-analysis-database-backup.lock -c 'cd $PROJECT_ROOT && PYTHONPATH=$PROJECT_ROOT $PYTHON_BIN scripts/run_database_backup.py --output-dir /root/stock-analysis-backups/automated --retention-count 7 >> $LOG_DIR/database_backup.log 2>&1'"
...
SENTIMENT_V06_SNAPSHOT_JOB="32 9 * * 1-5 cd $PROJECT_ROOT && PYTHONPATH=$PROJECT_ROOT $PYTHON_BIN scripts/materialize_sentiment_candidate_snapshot.py --strategy-id a_share_sentiment_v06 --allow-shadow >> $LOG_DIR/sentiment_candidate_snapshot_v06.log 2>&1
2 11 * * 1-5 cd $PROJECT_ROOT && PYTHONPATH=$PROJECT_ROOT $PYTHON_BIN scripts/materialize_sentiment_candidate_snapshot.py --strategy-id a_share_sentiment_v06 --allow-shadow >> $LOG_DIR/sentiment_candidate_snapshot_v06.log 2>&1
32 13,14 * * 1-5 cd $PROJECT_ROOT && PYTHONPATH=$PROJECT_ROOT $PYTHON_BIN scripts/materialize_sentiment_candidate_snapshot.py --strategy-id a_share_sentiment_v06 --allow-shadow >> $LOG_DIR/sentiment_candidate_snapshot_v06.log 2>&1
57 18 * * 1-5 cd $PROJECT_ROOT && PYTHONPATH=$PROJECT_ROOT $PYTHON_BIN scripts/materialize_sentiment_candidate_snapshot.py --strategy-id a_share_sentiment_v06 --allow-shadow >> $LOG_DIR/sentiment_candidate_snapshot_v06.log 2>&1"
...
FILTERED_CRON="$(printf '%s\n' "$CURRENT_CRON" | grep -v 'run_database_backup.py' | grep -v 'backfill_kline_from_realtime_snapshot.py' | grep -v 'run_realtime_lifecycle.py' | grep -v 'run_market_opinion_lifecycle.py' | grep -v 'materialize_sentiment_candidate_snapshot.py' | grep -v 'refresh_operational_read_models.py' | grep -v 'run_stock_basic_sync.py' | grep -v 'run_adj_factor_daily_update.py' | grep -v 'run_moneyflow_daily_update.py' | grep -v 'run_chip_daily_update.py' | grep -v 'run_kline_daily_update.py' | grep -v 'refresh_stock_technical_feature_daily.py' | grep -v 'run_kline_history_backfill.py' | grep -v 'run_fundamental_daily_update.py' | grep -v 'run_valuation_daily_update.py' | grep -v 'backfill_pe_from_baidu_valuation.py' | grep -v 'refresh_stock_status_snapshot.py' | grep -v 'run_factor_input_daily_update.py' | grep -v 'run_strategy_factor_ci_daily_update.py' | grep -v 'run_strategy_factor_evaluation_v2.py' | grep -v 'run_market_context_daily_update.py' | grep -v 'run_market_timing_daily_update.py' | grep -v 'run_market_timing_v20.py' | grep -v 'run_market_scenario_forecast.py' | grep -v 'run_sentiment_daily_update.py' | grep -v 'run_stock_status_pit_backfill.py' | grep -v 'run_fundamental_pit_backfill.py' | grep -v 'run_index_constituent_pit_backfill.py' | grep -v 'run_data_quality_audit.py' | grep -v 'run_market_opinion_update.py' | grep -v 'run_realtime_snapshot_update.py' | grep -v 'run_portfolio_etf_quote_update.py' | grep -v 'run_market_fund_flow_update.py' | grep -v 'run_ths_concept_hot_update.py' | grep -v 'run_realtime_moneyflow_update.py' | grep -v 'run_stock_popularity_update.py' | grep -v 'run_job_retention.py' | grep -v 'run_strategy_forward_observation.py' | grep -v 'run_automatic_strategy_observation.py' | grep -v 'run_strategy_forward_outcomes.py' | grep -v 'run_etf_rotation_data_sync.py' | grep -v 'run_etf_rotation_signal.py' | grep -v 'run_etf_rotation_outcomes.py' || true)"
NEW_CRON="$(printf '%s\n' "$FILTERED_CRON" "$DATABASE_BACKUP_JOB" "$EOD_REALTIME_BACKFILL_JOB" "$REALTIME_LIFECYCLE_JOB" "$STOCK_BASIC_JOB" "$DAILY_JOB" "$TECHNICAL_FEATURE_JOB" "$ADJ_FACTOR_JOB" "$MONEYFLOW_DAILY_JOB" "$CHIP_DAILY_JOB" "$BACKFILL_JOB" "$FUNDAMENTAL_JOB" "$VALUATION_JOB" "$PE_BAIDU_BACKFILL_JOB" "$STATUS_SNAPSHOT_JOB" "$FACTOR_INPUT_JOB" "$STRATEGY_FACTOR_CI_JOB" "$STRATEGY_FACTOR_EVALUATION_JOB" "$ETF_ROTATION_DATA_JOB" "$ETF_ROTATION_SIGNAL_JOB" "$ETF_ROTATION_OUTCOME_JOB" "$MARKET_CONTEXT_JOB" "$MARKET_TIMING_JOB" "$MARKET_TIMING_V20_JOB" "$MARKET_SCENARIO_JOB" "$SENTIMENT_JOB" "$STOCK_STATUS_PIT_JOB" "$FUNDAMENTAL_PIT_JOB" "$INDEX_CONSTITUENT_PIT_JOB" "$DATA_QUALITY_JOB" "$JOB_RETENTION_JOB" "$FORWARD_OUTCOME_JOB" "$MARKET_OPINION_JOB" "$MARKET_OPINION_LIFECYCLE_JOB" "$SENTIMENT_SNAPSHOT_JOB" "$SENTIMENT_V06_SNAPSHOT_JOB" "$OPERATIONAL_READ_MODELS_JOB" "$AUTOMATIC_OBSERVATION_JOB" "$REALTIME_SNAPSHOT_JOB" "$PORTFOLIO_ETF_QUOTE_JOB" "$MARKET_FUND_FLOW_JOB" "$THS_CONCEPT_HOT_JOB" "$REALTIME_MONEYFLOW_JOB" "$STOCK_POPULARITY_JOB" | awk 'NF && !seen[$0]++')"
