...
FILTERED_CRON="$(printf '%s\n' "$CURRENT_CRON" | grep -v 'run_kline_daily_update.py' | grep -v 'run_kline_history_backfill.py' | grep -v 'run_fundamental_daily_update.py' | grep -v 'run_valuation_daily_update.py' | grep -v 'backfill_pe_from_baidu_valuation.py' | grep -v 'refresh_stock_status_snapshot.py' | grep -v 'run_factor_input_daily_update.py' | grep -v 'run_market_context_daily_update.py' | grep -v 'run_market_timing_daily_update.py' | grep -v 'run_sentiment_daily_update.py' | grep -v 'run_market_opinion_update.py' | grep -v 'run_realtime_snapshot_update.py' | grep -v 'run_market_fund_flow_update.py' | grep -v 'run_ths_concept_hot_update.py' | grep -v 'run_realtime_moneyflow_update.py' | grep -v 'run_stock_popularity_update.py' || true)"
NEW_CRON="$(printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' "$FILTERED_CRON" "$DAILY_JOB" "$BACKFILL_JOB" "$FUNDAMENTAL_JOB" "$VALUATION_JOB" "$PE_BAIDU_BACKFILL_JOB" "$STATUS_SNAPSHOT_JOB" "$FACTOR_INPUT_JOB" "$MARKET_CONTEXT_JOB" "$MARKET_TIMING_JOB" "$SENTIMENT_JOB" "$MARKET_OPINION_JOB" "$REALTIME_SNAPSHOT_JOB" "$MARKET_FUND_FLOW_JOB" "$THS_CONCEPT_HOT_JOB" "$REALTIME_MONEYFLOW_JOB" "$STOCK_POPULARITY_JOB" | awk '!seen[$0]++')"
