...
def main() -> None:
    ...
    try:
        result = AdjFactorSync().run(start_date, end_date, pause_seconds=args.pause_seconds)
        payload = {**metadata, **result}
        logger.finish("adj_factor_daily_update", run_id, "success", f"adj factor update completed, rows={result.get('rows_synced', 0)}", payload)
