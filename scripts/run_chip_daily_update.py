...
from app.data_ingestion.chip_sync import ChipSync
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Daily update Tushare cyq_perf into stock_chip_daily")
...
def main() -> None:
    ...
    if not start_date or not end_date:
        raise RuntimeError("no trade dates found for chip update")
    logger = TaskRunLogger()
    run_id = f"chip_daily_update_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    metadata = {"start_date": start_date, "end_date": end_date, "recent_trade_days": args.recent_trade_days}
    logger.start("chip_daily_update", run_id, metadata)
    try:
        result = ChipSync().run(start_date, end_date, pause_seconds=args.pause_seconds)
        payload = {**metadata, **result}
        logger.finish("chip_daily_update", run_id, "success", f"chip update completed, rows={result.get('rows_synced', 0)}", payload)
        print(json.dumps(payload, ensure_ascii=False))
    except Exception as exc:
        logger.finish("chip_daily_update", run_id, "failed", str(exc)[:500], metadata)
        raise
