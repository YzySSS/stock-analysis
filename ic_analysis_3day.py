...
def calculate_factors(cursor, code, date, stock_data, hist_prices):
    """计算所有因子值"""
    factors = {}
    ...
    try:
        price_now = float(hist_prices[0])
        price_20d = float(hist_prices[20])
    except (TypeError, ValueError):
        return None
    
    roe = float(stock_data['roe']) if stock_data.get('roe') else None
    pe = float(stock_data['pe']) if stock_data.get('pe') else None
    turnover = float(stock_data['turnover']) if stock_data.get('turnover') else None
    ...
    # 1. Value (PE)
    factors['value_pe'] = float(-pe) if pe and pe > 0 else -50.0
    
    # 2. Quality (ROE)
    if roe is not None:
        factors['quality_roe'] = float(roe)
    else:
        factors['quality_roe'] = float(50 - np.std(returns[-20:])) if len(returns) >= 20 else 10.0
    ...
    # 4. LowVol
    if len(returns) >= 60:
        factors['lowvol_60d'] = float(-np.std(returns[-60:]))
    elif len(returns) >= 20:
        factors['lowvol_60d'] = float(-np.std(returns[-20:]) * np.sqrt(3))
    else:
        factors['lowvol_60d'] = -5.0
    
    # 5. Turnover
    factors['turnover'] = float(-turnover) if turnover else -5.0
...
def main():
    ...
    # 采样 (每3天)
    sample_dates = trade_dates[::3][20:-3]  # 跳过前20天和最后3天确保有未来数据
    ...
    factor_list = ['value_pe', 'quality_roe', 'momentum_20d', 'reversal_20d', 'lowvol_60d', 'turnover']
    ...
    for i, date in enumerate(sample_dates):
        ...
        try:
            ...
            for row in stocks:
                ...
                # 获取3日后的价格
                future_price = get_future_price(cursor, code, date, trade_dates, 3)
                ...
                # 计算因子
                factors = calculate_factors(cursor, code, date, stock_data, hist_prices)
                if factors is None:
                    continue
    ...
    for fname in factor_list:
        ...
        print(f"  IC>0比例: {stats['win_rate']:.1%}")
    ...
    if valid_factors:
        ...
        print(f"\n🏆 最强因子: {factor_names.get(best_factor[0], best_factor[0])}")
    ...
    # 保存结果
    output_file = '/root/.openclaw/workspace/股票分析项目/ic_analysis_3day_results.json'
